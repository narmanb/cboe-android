...
bool loc_off_world(location p1) {
	if(is_out())  return !univ.out.is_on_map(p1.x, p1.y);
	return !univ.town.is_on_map(p1.x, p1.y);
}

bool loc_off_act_area(location p1) {
	if(is_out()) return !univ.out->is_on_map(p1);
	if(!univ.town.is_on_map(p1.x, p1.y)) return true;
	return !univ.town->in_town_rect.contains(p1);
}
