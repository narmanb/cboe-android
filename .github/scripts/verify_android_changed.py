...
if len(sys.argv) != 3:
    raise SystemExit("usage: verify_android_changed.py BEFORE.raw AFTER.raw")

w1, h1, before = load(sys.argv[1])
w2, h2, after = load(sys.argv[2])
if (w1, h1) != (w2, h2):
    raise SystemExit(f"screenshot dimensions changed: {w1}x{h1} -> {w2}x{h2}")
...
print(f"changed-pixel ratio: {changed_ratio:.4f}")
print(f"after-screen nonblack ratio: {nonblack_ratio:.4f}")
if nonblack_ratio < 0.08:
    raise SystemExit("after screen is effectively black")
if changed_ratio < 0.01:
    raise SystemExit("screen did not visibly change after opening the dialog")
